// GameCode.cpp	- KF6017 Software Architecture for Games	
// Andrew Cobb
// w16015479
...
{
	pOM = new ObjectManager();
	music = MySoundEngine::GetInstance()->LoadWav(L"Music.wav");
	MySoundEngine* pSE = MySoundEngine::GetInstance();
	pSE->Play(music);
	pSE->SetVolume(music, 0);
	pLM = new LevelManager(pOM);
	pLM->Initialise();
	pOM->AddObject(pLM);
...
ErrorType Game::Update()
{
	...
	pOM->UpdateAll(0.02); //Updates each Object in game, inherited from ObjectManager
	pOM->RenderAll(); //Renders each Object in game, inherited from ObjectManager
	pOM->DeleteInactive(); //Deletes All Objects that are inactive
	pOM->CheckAllCollisions();
